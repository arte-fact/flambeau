...
#define IMQ5_0_BLOCK_THREADS 256
#define IMQ5_0_WARPS (IMQ5_0_BLOCK_THREADS / WARP_SIZE)
#define IMQ5_0_INT32_PER_BLOCK 4
#define IMQ5_0_BLOCKS_PER_ITER (IMQ5_0_BLOCK_THREADS / IMQ5_0_INT32_PER_BLOCK)

static __device__ __forceinline__ int flambeau_imq5_0_dp4a(int a, int b, int c) {
...
extern "C" __global__ void flambeau_indexed_moe_mmvq_q5_0_q8_1(
    ...
    for (int b = block_idx; b < n_blocks_per_row; b += IMQ5_0_BLOCKS_PER_ITER) {
        ...
        int sumi_nib = 0;
        sumi_nib = flambeau_imq5_0_dp4a(vi_lo, u_lo, sumi_nib);
        sumi_nib = flambeau_imq5_0_dp4a(vi_hi, u_hi, sumi_nib);
        int sumi_bit = 0;
        sumi_bit = flambeau_imq5_0_dp4a(bit_lo, u_lo, sumi_bit);
        sumi_bit = flambeau_imq5_0_dp4a(bit_hi, u_hi, sumi_bit);
    ...
    __shared__ float s_warp[IMQ5_0_WARPS];
    if (lane == 0) {
        s_warp[warp] = acc;
    }
    __syncthreads();
    ...
    if (warp == 0) {
        float v = (lane < IMQ5_0_WARPS) ? s_warp[lane] : 0.0f;
        #pragma unroll
        for (int off = IMQ5_0_WARPS / 2; off > 0; off >>= 1) {

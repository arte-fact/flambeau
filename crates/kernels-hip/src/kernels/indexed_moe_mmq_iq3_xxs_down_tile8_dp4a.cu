// indexed_moe_mmq_iq3_xxs_down_tile8_dp4a — IQ3_XXS MoE MMQ tile8 down.
// Phase 4 Slice D.
...
void flambeau_indexed_moe_mmq_iq3_xxs_down_tile8_dp4a_q8_1(
    const flambeau_block_iq3_xxs* __restrict__ down_w,
        ...
        for (int sub = 0; sub < q8_per_super; ++sub) {
            ...
            if (row_ok) {
                ...
                for (int l = 0; l < 4; ++l) {
                    ...
                    const uint8_t sb = KSIGNS_IQ2XS[(aux32 >> (7 * l)) & 0x7F];
                    v[2 * l + 0] = apply_signs_packed(g1, sb        & 0x0F);
                    v[2 * l + 1] = apply_signs_packed(g2, (sb >> 4) & 0x0F);
                }
            }

// mmq_f16_tile — tile-M MMQ for F16 weight × Q8_1 activation.
// The trivial multi-row variant (same per-block math as the
// MMVQ, grid.y = n_tokens) is pure launch-overhead saving, no compute
// amortisation, leaving the F16 path well below the MMQ path.
...
// Key difference from the null Y-LDS port on Q4_K tile8: Y is read

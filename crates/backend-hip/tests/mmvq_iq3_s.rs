...
fn run_mmvq_iq3_s(
    ...
    let mut x_dequant = vec![0.0f32; total_elems];
    {
        ...
        flambeau_quant::dequantize_into(flambeau_quant::GgmlDType::Iq3S, raw, &mut x_dequant)
            .unwrap();
    }
    ...
    unsafe {
        dev.dealloc(d_x, x_blocks.len() * std::mem::size_of::<BlockIq3S>())
            .unwrap();
...
fn mmvq_iq3_s_small() {
    ...
    let (got, reference) = run_mmvq_iq3_s(
        4,
        QK_K,
        0xC0FFEE,
        "mmvq_iq3_s",
        "flambeau_mmvq_iq3_s_q8_1",
        1,
    );
    let err = max_rel_err(&got, &reference);
    eprintln!(
        "[mmvq_iq3_s 4×{QK_K}] err={err:.3e}, tol={:.3e}",
        tol_for(QK_K)
    );
...
fn mmvq_iq3_s_r2_small() {
    ...
    let (got, reference) = run_mmvq_iq3_s(
        ...
        0xC0FFEE,
        "mmvq_iq3_s_r2",
        "flambeau_mmvq_iq3_s_r2_q8_1",
        2,
    );
    let err = max_rel_err(&got, &reference);
    eprintln!(
        "[mmvq_iq3_s_r2 4×{QK_K}] err={err:.3e}, tol={:.3e}",
        tol_for(QK_K)
    );
    assert!(err <= tol_for(QK_K), "err {err:.3e}");
}
...
fn mmvq_iq3_s_r2_2048() {
    ...
    let err = max_rel_err(&got, &reference);
    eprintln!(
        "[mmvq_iq3_s_r2 8×{k}] err={err:.3e}, tol={:.3e}",
        tol_for(k)
    );
    assert!(err <= tol_for(k), "err {err:.3e}");
}
...
fn mmvq_iq3_s_r2_odd_rows() {
    ...
    let err = max_rel_err(&got, &reference);
    eprintln!(
        "[mmvq_iq3_s_r2 7×{k}] err={err:.3e}, tol={:.3e}",
        tol_for(k)
    );
    assert!(err <= tol_for(k), "err {err:.3e}");
}

...
//!
//! Q/K/V buffers are deterministic small F16 values so the kernel output
//! stays in a narrow range. We rely on F16 bit-equality (via `assert_eq`
//! on the raw `u16` bits after cast) — the same kernel launched with the
//! same inputs on gfx906 should produce identical outputs regardless of
//! whether it came through a graph replay or direct dispatch.

//! S8-B-A — gemma4 pipeline-parallel prefill smoke. Synthetic 2-rank
//! fixture, 4 layers (2 per rank), L=8-token prefill chunk + per-rank
//! KV cache growth verification. Skipped when fewer than 2 HIP devices.
...
fn cluster_or_skip() -> Option<HipCluster> {
    ...
    if (n as usize) < N_RANKS {
        eprintln!(
            "skipping PP prefill smoke — need {N_RANKS} HIP devices but only {n}"
        );
        return None;
    }
...
fn build_driver(cluster: HipCluster) -> Gemma4PpDriver {
...
#[test]
fn forward_prefill_pp_smoke() {
    let Some(cluster) = cluster_or_skip() else { return; };
    let mut driver = build_driver(cluster);
    ...
    let l3_local = driver.stages[1].kv_caches[1]
        ...
        .current_tokens();
    assert_eq!(l3_local, PROMPT_LEN, "rank 1 last layer KV after prefill");

    driver.dispose().expect("dispose");
}

#[test]
fn forward_prefill_pp_chunked_smoke() {
    // Two consecutive chunks share state via `start_position`. Each
    // chunk is L=4 within the same prompt.
    let Some(cluster) = cluster_or_skip() else { return; };
    let mut driver = build_driver(cluster);
    ...
    let kv_after = driver.stages[0].kv_caches[0]
        ...
        .expect("KV")
        .current_tokens();
    assert_eq!(kv_after, 8, "KV after 2 chunks of L=4");

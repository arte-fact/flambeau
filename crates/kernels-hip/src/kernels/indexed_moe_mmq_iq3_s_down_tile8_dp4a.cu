// indexed_moe_mmq_iq3_s_down_tile8_dp4a — IQ3_S MoE MMQ tile8 down.
...
void flambeau_indexed_moe_mmq_iq3_s_down_tile8_dp4a_q8_1(
    const flambeau_block_iq3_s* __restrict__ down_w,
        ...
        for (int sub = 0; sub < q8_per_super; ++sub) {
            ...
            if (row_ok) {
                ...
                for (int l = 0; l < 4; ++l) {
                    ...
                    const uint8_t signs = bx->signs[sub * 4 + l];
                    v[2 * l + 0] = apply_signs_packed(g1, signs        & 0x0F);
                    v[2 * l + 1] = apply_signs_packed(g2, (signs >> 4) & 0x0F);
                }
            }
            ...
            for (int c = 0; c < TILE_N; ++c) {
                ...
                sums[c] += d * d8 * (float) sumi * db_no_d;
            }
        }
    }

//! End-to-end correctness for `flambeau_mmvq_iq4_nl_q8_1` on MI50 vs CPU
//! dequant + f32 matmul (with Q8_1 activation round-trip to isolate kernel
//! arithmetic from quant-representation noise).
//!
//! Generates IQ4_NL blocks by direct byte randomisation — the kernel's
//! arithmetic operates on whatever bit pattern the blocks hold (the codes
//! are valid 4-bit LUT indices for any byte), so the cert is valid
//! regardless of whether the weights came from a real `iq4nl` quantiser.

#![expect(
    clippy::undocumented_unsafe_blocks,
    reason = "test fixture — every `unsafe {}` below is a kernel launch or `memcpy_async`               whose invariant is uniform: host/device buffers live for the bounded               `synchronize()` that follows, pointers are freshly allocated above, kernel               ABIs match kernels-hip."
)]
#![expect(
...
use flambeau_quant::{BlockIq4Nl, BlockQ8_1, QK4_0, QK8_0};
use half::f16;

const QK8: usize = QK8_0; // 32
const QK_NL: usize = QK4_0; // 32
...
fn random_iq4_nl_block(seed: u64, idx: usize) -> BlockIq4Nl {
    let bytes = seeded_bytes(seed ^ ((idx as u64).wrapping_mul(0x9E3779B97F4A7C15)), 18);
    // Keep d in a realistic-GGUF range so the F32 dot product stays well
    // away from F32's relative-precision floor. Real iq4nl scales live
    // around 1e-2 because the quantiser normalises weights to amax ≈ 1.
...
fn run_mmvq_iq4_nl(
    ...
    let mut x_blocks: Vec<BlockIq4Nl> = Vec::with_capacity(n_rows * blocks_per_row);
    for i in 0..(n_rows * blocks_per_row) {
        x_blocks.push(random_iq4_nl_block(seed, i));
    }
...

    {
        ...
        let n_blk_i = blocks_per_row as i32;

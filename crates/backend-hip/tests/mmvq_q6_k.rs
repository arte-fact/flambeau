...
use flambeau_quant::{BlockQ6K, QK_K};
use half::f16;

fn random_q6k_block(seed: u64, idx: usize) -> BlockQ6K {
    let bytes = seeded_bytes(seed ^ ((idx as u64).wrapping_mul(0x9E3779B97F4A7C15)), 210);
    let mut ql = [0u8; QK_K / 2];
    ql.copy_from_slice(&bytes[0..QK_K / 2]);
    let mut qh = [0u8; QK_K / 4];
    qh.copy_from_slice(&bytes[QK_K / 2..QK_K / 2 + QK_K / 4]);
    // Scales are i8 in Q6_K — keep them in a realistic small signed range.
    let mut scales = [0i8; QK_K / 16];
    for (i, b) in bytes[QK_K / 2 + QK_K / 4..QK_K / 2 + QK_K / 4 + QK_K / 16]
        .iter()
        .enumerate()
    {
        // map to [-32, 32] — real Q6_K scales live in a similar range.
        scales[i] = ((*b as i32 % 65) - 32) as i8;
    }
    let d = f16::from_f32((bytes[QK_K / 2 + QK_K / 4 + QK_K / 16] as f32 / 255.0) * 0.05 + 0.01);
    BlockQ6K { ql, qh, scales, d }
}
...
fn run(n_rows: usize, k: usize, seed: u64) -> (Vec<f32>, Vec<f32>) {
    ...
    let m_bytes = kernels::hsaco("mmvq_q6_k").unwrap();
...
fn mmvq_q6_k_small() {
    ...
    eprintln!("[mmvq_q6_k 4×{QK}] err={err:.3e}, tol={tol:.3e}");
    assert!(err <= tol);
}
...
fn mmvq_q6_k_qwen_2048() {
    ...
    let err = max_rel_err(&got, &reference);
    let tol = cert_tol(k);
    eprintln!("[mmvq_q6_k 8×{k}] err={err:.3e}, tol={tol:.3e}");
    assert!(err <= tol);
}
...
fn mmvq_q6_k_5120() {
    ...
    let err = max_rel_err(&got, &reference);
    let tol = cert_tol(k);
    eprintln!("[mmvq_q6_k 16×{k}] err={err:.3e}, tol={tol:.3e}");
    assert!(err <= tol);
}
...
fn mmvq_q6_k_15360() {
    ...
    let err = max_rel_err(&got, &reference);
    let tol = cert_tol(k);
    eprintln!("[mmvq_q6_k 4×{k}] err={err:.3e}, tol={tol:.3e}");
    assert!(err <= tol);
}

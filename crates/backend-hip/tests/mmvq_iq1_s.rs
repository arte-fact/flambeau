...
fn random_block(seed: u64, idx: usize) -> BlockIq1S {
    ...
    let mut qs = [0u8; QK_K / 8];
    qs.copy_from_slice(&bytes[2..2 + QK_K / 8]);
    let mut qh = [0u8; 2 * QK_K / 32];
    qh.copy_from_slice(&bytes[2 + QK_K / 8..]);
    BlockIq1S { d, qs, qh }
}
...
fn ref_mm(w: &[f32], y: &[f32], rows: usize, k: usize) -> Vec<f32> {
    (0..rows)
        .map(|r| (0..k).map(|j| (w[r * k + j] * y[j]) as f64).sum::<f64>() as f32)
        .collect()
}
...
fn run(
    ...
    let d_d = dev.alloc(rows * 4).unwrap();
    {
        ...
        let p1: u64 = d_y.as_usize() as u64;
        let p2: u64 = d_yq.as_usize() as u64;
        let n = k as i32;
        a.push(&p1);
        a.push(&p2);
        a.push(&n);
        unsafe {
            kq.launch(
                dev.default_stream(),
                LaunchCfg::one_d(yb as u32, QK8 as u32),
                a,
            )
            .unwrap();
        }
    ...
    {
        let mut a = KernelArgs::new();
        let p1: u64 = d_x.as_usize() as u64;
        let p2: u64 = d_yq.as_usize() as u64;
        let p3: u64 = d_d.as_usize() as u64;
        let nr = rows as i32;
        let ns = nsb as i32;
        a.push(&p1);
        a.push(&p2);
        ...
        unsafe {
            km.launch(
                dev.default_stream(),
                LaunchCfg::one_d(rows.div_ceil(rpb) as u32, 64),
                a,
            )
            .unwrap();
        }
    ...
    dev.default_stream().synchronize().unwrap();
    unsafe {
        dev.dealloc(d_x, blocks.len() * std::mem::size_of::<BlockIq1S>())
            .unwrap();
        dev.dealloc(d_y, y.len() * 4).unwrap();
        dev.dealloc(d_yq, yb * std::mem::size_of::<BlockQ8_1>())
            .unwrap();
        dev.dealloc(d_d, rows * 4).unwrap();
    }
    (dst, ref_mm(&deq, &q8_1_rt(&y), rows, k))
}
fn err(g: &[f32], r: &[f32]) -> f32 {
    g.iter()
        .zip(r)
...
fn iq1_s_small() {
    ...
    let (g, r) = run(
        ...
        0xC0FFEE,
        "mmvq_iq1_s",
        "flambeau_mmvq_iq1_s_q8_1",
        1,
    );
    let e = err(&g, &r);
    eprintln!("[iq1_s 4x{QK_K}] err={e:.3e}");
    assert!(e <= tol(QK_K));
}
#[test]
fn iq1_s_2048() {
    ...
    let e = err(&g, &r);
    eprintln!("[iq1_s 8x2048] err={e:.3e}");
    assert!(e <= tol(2048));
}
#[test]
fn iq1_s_r2_small() {
    ...
    let (g, r) = run(
        ...
        0xC0FFEE,
        "mmvq_iq1_s_r2",
        "flambeau_mmvq_iq1_s_r2_q8_1",
        2,
    );
    let e = err(&g, &r);
    eprintln!("[iq1_s_r2 4x{QK_K}] err={e:.3e}");
    assert!(e <= tol(QK_K));
}
#[test]
fn iq1_s_r2_2048() {
    ...
    let e = err(&g, &r);
    eprintln!("[iq1_s_r2 8x2048] err={e:.3e}");
    assert!(e <= tol(2048));
}
#[test]
fn iq1_s_r2_odd() {
    ...
    let e = err(&g, &r);
    eprintln!("[iq1_s_r2 7x1024] err={e:.3e}");
    assert!(e <= tol(1024));
}

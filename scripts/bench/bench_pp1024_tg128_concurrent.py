#!/usr/bin/env python3
"""Concurrent pp~1024 / tg=128 bench. Fires N streaming chat
completions in parallel with the same ~1024-token technical prompt
and reports per-consumer (per-stream) numbers plus the cumulated
aggregate.
...
def main():
    print(f"BENCH-pp1024-tg128-concurrent: N={N} tg={TG} model={MODEL}")
    results = [None] * N
    t_start = time.monotonic()
    threads = [threading.Thread(target=fire, args=(i, results), name=f"c{i}") for i in range(N)]
    ...
    print()
    print("per-consumer:")
    print(f"  {'#':>3}  {'TTFT (ms)':>10}  {'wall (s)':>9}  {'n_tok':>6}  {'decode tok/s':>13}")
    for i, r in enumerate(results):
        if r is None or "error" in r:
            print(f"  {i:>3}  {'-':>10}  {'-':>9}  {'-':>6}  {'-':>13}  {r}")
        else:
            ttft = f"{r['ttft_ms']:.0f}" if r["ttft_ms"] is not None else "-"
            print(f"  {i:>3}  {ttft:>10}  {r['wall_s']:>9.2f}  {r['n_tok']:>6}  {r['decode_tps']:>13.2f}")
    ...
    print(f"  effective concurrency factor:   {sum_decode_tps / (ok[0]['decode_tps'] if ok else 1):.2f}x  (vs slowest stream)")

...
fn random_block(seed: u64, idx: usize) -> BlockIq1M {
    ...
    qs.copy_from_slice(&bytes[0..QK_K / 8]);
    let mut qh = [0u8; QK_K / 16];
    qh.copy_from_slice(&bytes[QK_K / 8..QK_K / 8 + QK_K / 16]);
    let mut scales = [0u8; QK_K / 32];
    scales.copy_from_slice(&bytes[QK_K / 8 + QK_K / 16..]);
...
fn run(
    ...
    dev.default_stream().synchronize().unwrap();
    unsafe {
        dev.dealloc(d_x, blocks.len() * std::mem::size_of::<BlockIq1M>())
            .unwrap();
...
fn iq1_m_small() {
    ...
    let (g, r) = run(
        ...
        0xC0FFEE,
        "mmvq_iq1_m",
        "flambeau_mmvq_iq1_m_q8_1",
        1,
    );
    let e = err(&g, &r);
    eprintln!("[iq1_m 4x{QK_K}] err={e:.3e}");
    assert!(e <= tol(QK_K));
}
#[test]
fn iq1_m_2048() {
    ...
    let e = err(&g, &r);
    eprintln!("[iq1_m 8x2048] err={e:.3e}");
    assert!(e <= tol(2048));
}
#[test]
fn iq1_m_r2_small() {
    ...
    let (g, r) = run(
        ...
        0xC0FFEE,
        "mmvq_iq1_m_r2",
        "flambeau_mmvq_iq1_m_r2_q8_1",
        2,
    );
    let e = err(&g, &r);
    eprintln!("[iq1_m_r2 4x{QK_K}] err={e:.3e}");
    assert!(e <= tol(QK_K));
}
#[test]
fn iq1_m_r2_2048() {
    ...
    let e = err(&g, &r);
    eprintln!("[iq1_m_r2 8x2048] err={e:.3e}");
    assert!(e <= tol(2048));
}
#[test]
fn iq1_m_r2_odd() {
    ...
    let e = err(&g, &r);
    eprintln!("[iq1_m_r2 7x1024] err={e:.3e}");
    assert!(e <= tol(1024));
}

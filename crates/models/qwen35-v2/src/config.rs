...
#[derive(Error, Debug)]
pub enum Qwen35V2ConfigError {
    #[error("missing required GGUF key: {0}")]
    MissingKey(String),
    #[error("expected `qwen35` architecture, got {got:?}")]
...
pub struct Qwen35V2Config {
    ...
    pub gdn: GdnDims,
    /// Layer `il` is recurrent (GDN) iff `(il + 1) % full_attention_interval != 0`.
    pub full_attention_interval: usize,
}

impl Qwen35V2Config {
    pub fn from_gguf(file: &GgufFile) -> Result<Self, Qwen35V2ConfigError> {
        let arch = file.architecture().map(|s| s.to_string());
        if arch.as_deref() != Some("qwen35") {
            return Err(Qwen35V2ConfigError::WrongArchitecture { got: arch });
        }
        let key = |suffix: &str| format!("qwen35.{suffix}");
        let mk_missing = |suffix: &str| Qwen35V2ConfigError::MissingKey(key(suffix));
        ...
        let vocab_size = file
            ...
            .map(|v| v as usize)
            .ok_or(Qwen35V2ConfigError::MissingTokenEmbd)?;

//! End-to-end correctness for `flambeau_mmvq_iq1_s_q8_1` on MI50.
...
use flambeau_quant::{BlockIq1S, BlockQ8_1, QK8_0, QK_K};
use half::f16;
...
fn random_block(seed: u64, idx: usize) -> BlockIq1S {
    let bytes = seeded_bytes(seed ^ ((idx as u64).wrapping_mul(0x9E3779B97F4A7C15)), BLOCK_BYTES);
    // IQ1_S scale = 2*nibble + 1 (max 15). Cap d very tight.
    let d = f16::from_f32((bytes[0] as f32 / 255.0) * 0.001 + 0.0001);
    let mut qs = [0u8; QK_K / 8]; qs.copy_from_slice(&bytes[2..2 + QK_K / 8]);
    let mut qh = [0u8; 2 * QK_K / 32]; qh.copy_from_slice(&bytes[2 + QK_K / 8..]);
    BlockIq1S { d, qs, qh }
}
...
fn run(rows: usize, k: usize, seed: u64, stem: &'static str, entry: &'static str, rpb: usize) -> (Vec<f32>, Vec<f32>) {
    ...
    let blocks: Vec<BlockIq1S> = (0..rows*nsb).map(|i| random_block(seed, i)).collect();
    let mut deq = vec![0.0f32; rows*k];
    flambeau_quant::dequantize_into(flambeau_quant::GgmlDType::Iq1S, bytemuck::cast_slice(&blocks), &mut deq).unwrap();
    ...
    unsafe { dev.dealloc(d_x, blocks.len() * std::mem::size_of::<BlockIq1S>()).unwrap(); dev.dealloc(d_y, y.len() * 4).unwrap();
...
#[test] fn iq1_s_small() { if !maybe_skip() { return; } let (g,r) = run(4, QK_K, 0xC0FFEE, "mmvq_iq1_s", "flambeau_mmvq_iq1_s_q8_1", 1); let e = err(&g,&r); eprintln!("[iq1_s 4x{QK_K}] err={e:.3e}"); assert!(e <= tol(QK_K)); }
#[test] fn iq1_s_2048() { if !maybe_skip() { return; } let (g,r) = run(8, 2048, 0xFEEDFACE, "mmvq_iq1_s", "flambeau_mmvq_iq1_s_q8_1", 1); let e = err(&g,&r); eprintln!("[iq1_s 8x2048] err={e:.3e}"); assert!(e <= tol(2048)); }
#[test] fn iq1_s_r2_small() { if !maybe_skip() { return; } let (g,r) = run(4, QK_K, 0xC0FFEE, "mmvq_iq1_s_r2", "flambeau_mmvq_iq1_s_r2_q8_1", 2); let e = err(&g,&r); eprintln!("[iq1_s_r2 4x{QK_K}] err={e:.3e}"); assert!(e <= tol(QK_K)); }
#[test] fn iq1_s_r2_2048() { if !maybe_skip() { return; } let (g,r) = run(8, 2048, 0xFEEDFACE, "mmvq_iq1_s_r2", "flambeau_mmvq_iq1_s_r2_q8_1", 2); let e = err(&g,&r); eprintln!("[iq1_s_r2 8x2048] err={e:.3e}"); assert!(e <= tol(2048)); }
#[test] fn iq1_s_r2_odd() { if !maybe_skip() { return; } let (g,r) = run(7, 1024, 0xDEADBEEF, "mmvq_iq1_s_r2", "flambeau_mmvq_iq1_s_r2_q8_1", 2); let e = err(&g,&r); eprintln!("[iq1_s_r2 7x1024] err={e:.3e}"); assert!(e <= tol(1024)); }

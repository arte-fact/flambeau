...
V1 is a vertical slice — one model, one arch — but **not** a "parity-first, tune-later" slice. The first commit targets max perf; multi-device (**PP-primary** under the PCIe-only rig constraint) is in, not deferred.

- **One model in V1: Qwen3.6-31B-A3B (arch `qwen35moe`, hybrid GDN + full-attn + MoE-with-shared-expert), Q4_K_S GGUF (~20 GB).** The hardest target — hybrid recurrent/attention + MoE + GQA + partial NeoX RoPE + mixed K-quants + shared expert + per-token sigmoid gate. Other models are V2+.
...
- **Multi-device is architectural, not V3.** `runtime::Mesh<N>` + collectives are V1.0 trait surface; RCCL collectives are V1.2. Every downstream component is mesh-generic from inception; `Mesh<1>` is a degenerate instance through the same trait. V1 ships on 1×, 2×, and 4× MI50 with a **`≥ 3.0×` PP (pipeline-parallel) scaling gate at `Mesh<4>`**. **Pipeline parallelism — not wide expert parallelism — is the V1 topology.** Rationale: the V1 rig is PCIe-only (no xGMI / no NVLink peer links), and the 2025–2026 consensus across vLLM, llama.cpp, and DeepEP literature is that TP>2 and wide-EP all-to-all are bandwidth-bound on PCIe (40–50% comm overhead measured; `llama.cpp` `split_mode=row` is 3–8× slower than `layer` on non-P2P topologies and has known long-context miscompiles). Decode-path design: each rank owns ~`num_layers / Mesh<N>::ranks` **contiguous** layers with its MoE experts held locally (no cross-rank routing). Stage-boundary transfer is one F16 hidden vector (~4 KB at `hidden=2048`) per token per hop, host-bounced via pinned-memory ping-pong DtoH/HtoD (the pattern candle landed in `hip_backend/cluster.rs`: 4 MiB chunks, 2 bufs/rank, ~6.75 GB/s measured on MI50 PCIe 3.0 x16 — plenty for 4 KB hand-offs). Wide-EP with router all-to-all is scoped to V2+ once an xGMI / NVLink rig is in play. RCCL bindings stay for future use but are **not** on the V1 decode hot path.
- **Perf bar is silicon, not llama.cpp.** Target ≥ 70% HBM utilisation on decode, ≥ 60 tok/s tg64 on 1× MI50, ≥ 600 tok/s pp512. Decode roofline estimate on 4× PP: ~5 GB active weights per stage / 1 TB/s HBM ≈ 1.6 ms/token/stage × 4 stages pipelined ≈ ~150 tok/s — the 60 tok/s tg64 gate leaves headroom for PP bubbles + PCIe hand-off latency (~10–50 µs per hop). llama.cpp is a correctness oracle and a lower-bound reference; if we match the silicon ceiling and they're higher, we file a diagnosis, we don't ship a regression to "catch up".

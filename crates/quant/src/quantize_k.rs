...
pub fn quantize_row_q4_k(x: &[f32], out: &mut Vec<u8>) {
    ...
    for i in 0..nb {
        ...
        // Emit: d (f16) + dmin (f16) + scales[12] + qs[128].
        out.extend_from_slice(&d_f16.to_bits().to_le_bytes());
        out.extend_from_slice(&dmin_f16.to_bits().to_le_bytes());
        out.extend_from_slice(&block_scales);
        // qs: pack low nibble at byte j (elem j), high nibble at byte j
        // (elem j + 32), grouped per 64-elem half-super-block.
        for j_off in (0..QK_K).step_by(64) {
            for l in 0..32 {
                out.push(local_l[j_off + l] | (local_l[j_off + l + 32] << 4));
            }
        }
    }
}
...
/// Output bytes: `hmask[32] + qs[64] + scales[12] + d (f16)`.
pub fn quantize_row_q3_k(x: &[f32], out: &mut Vec<u8>) {
    ...
    for i in 0..nb {
        ...
        // High bit → hmask (32 bytes, 1 bit per element across walking-`m`
        // selector).
        hmask.fill(0);
        ...
        // qs: pack 4 × 2-bit per byte across 64-elem strides.
        qs.fill(0);
        ...
        // Emit: hmask[32] + qs[64] + scales[12] + d (f16).
...
/// Output bytes: `scales[16] + qs[64] + d (f16) + dmin (f16)`.
pub fn quantize_row_q2_k(x: &[f32], out: &mut Vec<u8>) {
    ...
    for i in 0..nb {
        ...
        // qs: 4 × 2-bit per byte across 128-elem strides.
        let mut qs = [0u8; QK_K / 4];
        for j_off in (0..QK_K).step_by(128) {
            for l in 0..32 {
        ...
        // Emit: scales[16] + qs[64] + d (f16) + dmin (f16).
        out.extend_from_slice(&block_scales);
        out.extend_from_slice(&qs);
        out.extend_from_slice(&d_f16.to_bits().to_le_bytes());
        out.extend_from_slice(&dmin_f16.to_bits().to_le_bytes());

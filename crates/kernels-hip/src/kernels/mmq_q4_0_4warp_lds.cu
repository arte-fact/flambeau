...
// --- MMQ geometry (mirrors mmq_q4_1_4warp_lds) ---
#ifndef MMQ_Y
#define MMQ_Y 128
#endif
#ifndef MMQ_NWARPS
...
// LDS sizing — same as Q4_1 (we keep the half2 dm layout).
#define X_QS_INTS (MMQ_Y * (MMQ_TILE_NE_K + 1))
#define X_DM_H2S  (MMQ_Y * (MMQ_TILE_NE_K / QI4_0) + MMQ_Y / QI4_0)
...
// Q4_0 vec dot — bias-correction identity. dm4.x = d_x; ds8.x = d_y; ds8.y = y_s.
static __device__ __forceinline__ float q4_0_q8_1_dp4a_4(
    ...
    // d_x * (d_y * sumi - 8 * y_s)
    return dm4f.x * (ds8f.x * (float) sumi - 8.0f * ds8f.y);
}

// Load X tile — Q4_0 (no `m`). x_dm.y is set to 0 (unused in dot).
...
static __device__ __forceinline__ void load_tiles_q4_0(
    ...
    for (int i0 = 0; i0 < MMQ_Y; i0 += nrows * MMQ_NWARPS) {
        ...
        const flambeau_block_q4_0* bxi =
            (const flambeau_block_q4_0*) x + kbx0 + i * stride_row_x + kbx_safe;
    ...
    for (int i0 = 0; i0 < MMQ_Y; i0 += MMQ_NWARPS * rows_per_warp) {
        ...
        const flambeau_block_q4_0* bxi =
            (const flambeau_block_q4_0*) x + kbx0 + i * stride_row_x + kbxd_safe;
        // x_dm.x = d, x_dm.y = 0 (Q4_0 has no min field).
        const __half2 dm = kbxd_valid
            ? __floats2half2_rn((float) bxi->d, 0.0f)
            : __floats2half2_rn(0.0f, 0.0f);
        x_dm[i * (MMQ_TILE_NE_K / QI4_0) + i / QI4_0 + kbxd] = dm;
    }
}

// vec dot — same as Q4_1 except calls q4_0_q8_1_dp4a_4.
template <int mmq_x>
    ...
    for (int k01 = 0; k01 < MMQ_TILE_NE_K; k01 += QR4_0 * VDR_Q4_0_Q8_1_MMQ) {
        ...
        for (int j0 = 0; j0 < mmq_x; j0 += MMQ_NWARPS) {
            ...
            for (int i0 = 0; i0 < MMQ_Y; i0 += WARP_SIZE) {
                ...
                const __half2 dm4 =
                    x_dm[i * (MMQ_TILE_NE_K / QI4_0) + i / QI4_0 + k0 / (QR4_0 * QI4_0)];
...
static __device__ void mul_mat_q4_0_4warp_impl(
    ...
    const int n_blocks_x = ncols_x / QK4_0;
    const int kb0_stop = (n_blocks_x + blocks_per_iter - 1)
                         / blocks_per_iter * blocks_per_iter;
    const int i_max = nrows_x - it * MMQ_Y - 1;

    constexpr int q4_0_block_bytes = 18;  // sizeof(flambeau_block_q4_0)
    ...
    for (int kb0 = 0; kb0 < kb0_stop; kb0 += blocks_per_iter) {
        ...
        if (prefetch_valid) {
            ...
            const int64_t offset_x_bytes_next =
                ((int64_t) it * MMQ_Y * stride_row_x + kb0_next) * q4_0_block_bytes;
            x_prefetch_dummy = gfx906_prefetch_x_next(
                (const char*) vx,
                (int) offset_x_bytes_next,
                stride_row_x * q4_0_block_bytes);
        }
        ...
        vec_dot_q4_0_q8_1_dp4a<mmq_x>(x_qs, x_dm, tile_y, sum, 0);
        ...
        vec_dot_q4_0_q8_1_dp4a<mmq_x>(x_qs, x_dm, tile_y, sum, MMQ_TILE_NE_K);
...
void flambeau_mmq_q4_0_4warp_lds_q8_1(
    const void* __restrict__ vx,
    const void* __restrict__ vy,
    float*      __restrict__ dst,
    const int ncols_x,
    ...
    mul_mat_q4_0_4warp_impl</*mmq_x=*/64, /*need_check=*/false>(

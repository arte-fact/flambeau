...
fn run_mmvq_iq4_xs(
    ...
    let mut x_dequant = vec![0.0f32; total_elems];
    {
        ...
        flambeau_quant::dequantize_into(flambeau_quant::GgmlDType::Iq4Xs, raw, &mut x_dequant)
            .unwrap();
    }
    ...
    unsafe {
        dev.dealloc(d_x, x_blocks.len() * std::mem::size_of::<BlockIq4Xs>())
            .unwrap();
...
fn mmvq_iq4_xs_small() {
    ...
    let (got, reference) = run_mmvq_iq4_xs(
        4,
        QK_K,
        0xC0FFEE,
        "mmvq_iq4_xs",
        "flambeau_mmvq_iq4_xs_q8_1",
        1,
    );
    let err = max_rel_err(&got, &reference);
    eprintln!(
        "[mmvq_iq4_xs 4×{QK_K}] err={err:.3e}, tol={:.3e}",
        tol_for(QK_K)
    );
...
fn mmvq_iq4_xs_r2_small() {
    ...
    let (got, reference) = run_mmvq_iq4_xs(
        ...
        0xC0FFEE,
        "mmvq_iq4_xs_r2",
        "flambeau_mmvq_iq4_xs_r2_q8_1",
        2,
    );
    let err = max_rel_err(&got, &reference);
    eprintln!(
        "[mmvq_iq4_xs_r2 4×{QK_K}] err={err:.3e}, tol={:.3e}",
        tol_for(QK_K)
    );
    assert!(err <= tol_for(QK_K), "err {err:.3e}");
}
...
fn mmvq_iq4_xs_r2_2048() {
    ...
    let err = max_rel_err(&got, &reference);
    eprintln!(
        "[mmvq_iq4_xs_r2 8×{k}] err={err:.3e}, tol={:.3e}",
        tol_for(k)
    );
    assert!(err <= tol_for(k), "err {err:.3e}");
}
...
fn mmvq_iq4_xs_r2_odd_rows() {
    ...
    let err = max_rel_err(&got, &reference);
    eprintln!(
        "[mmvq_iq4_xs_r2 7×{k}] err={err:.3e}, tol={:.3e}",
        tol_for(k)
    );
    assert!(err <= tol_for(k), "err {err:.3e}");
}

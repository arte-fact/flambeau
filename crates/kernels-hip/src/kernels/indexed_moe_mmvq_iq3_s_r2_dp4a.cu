// indexed_moe_mmvq_iq3_s_r2_dp4a — IQ3_S MoE MMVQ with DP4A inner loop.
// Drop-in replacement for `indexed_moe_mmvq_iq3_s.cu` (scalar FP32).
// Same per-element math as dense `mmvq_iq3_s_dp4a.cu` plus the standard
// MoE wiring.
//
// Launch: blockDim = { 64 } (wave64), gridDim = { n_row_pairs,
// n_tokens * top_k, 1 }. 2 rows per block; 32 lanes per row covering
// (iqs_idx ∈ 0..7) × (l_grp ∈ 0..3). Per (iqs, l_grp): IQ3S_GRID
// lookups extended by qh 9th bit, separate signs[] byte read, two
// DP4As, scale 1 + 2*nibble.

#include "block_quant.cuh"
#include "../arch_primitives/gfx906.cuh"
#include "iq_grid.cuh"

static __device__ __forceinline__ int flambeau_indexed_moe_iq3_s_dp4a(
    int a, int b, int c
) {
    return __builtin_amdgcn_sdot4(a, b, c, false);
}

static __device__ __forceinline__ int flambeau_indexed_moe_iq3_s_apply_signs(
...
extern "C" __global__ void flambeau_indexed_moe_mmvq_iq3_s_r2_dp4a_q8_1(
    ...
    const flambeau_block_iq3_s* xrow =
    ...
    for (int sb = 0; sb < n_sb_per_row; ++sb) {
        ...
        const int grid_signed0 = flambeau_indexed_moe_iq3_s_apply_signs(grid0, signs0_pack);
        const int grid_signed1 = flambeau_indexed_moe_iq3_s_apply_signs(grid1, signs1_pack);
        ...
        int sumi = flambeau_indexed_moe_iq3_s_dp4a(grid_signed0, u0, 0);
        sumi     = flambeau_indexed_moe_iq3_s_dp4a(grid_signed1, u1, sumi);

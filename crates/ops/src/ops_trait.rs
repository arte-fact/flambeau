...
pub trait Ops {
    ...
    /// Gate-only half of `indexed_moe_mmq_q4_k_gate_up_tile8`. Same args
    /// minus the `up` side. Pair with `indexed_moe_mmq_q4_k_up_only_tile8`
    /// to reproduce the fused kernel's output at half the per-thread VGPR.

//! attention decode with Q8_0 KV cache, split-K (flash-decoding) — cert.
//! Two-pass split-K sibling of `sweep_attention_q8_kv`: same Q8-round-tripped
//! F32 reference, but launches the `attention_decode_q8_kv_splitk` chunk +
//! combine pair. Exercises ring_depth + chunk_base at 0 (absolute addressing).
...
/// (head_dim, n_heads_q, n_heads_kv) — same Q8 KV families as the single-pass cert.
const SHAPES: &[(usize, usize, usize)] = &[
    (128, 32, 4), // Qwen3.5
    (256, 16, 2), // Qwen3.6 / gemma4 SWA
];
...
pub fn run_sweep(repo_root: &Path) -> Result<Cert> {
    ...
    dev.bind()?;
    let kb = kernels::hsaco("attention_decode_q8_kv_splitk").unwrap();
    let module = HipModule::load(dev.id(), kb)?;
    let k_chunk: HipKernel<'_> = module.kernel("flambeau_attention_decode_q8_kv_splitk_chunk")?;
    ...
    for &(head_dim, n_heads_q, n_heads_kv) in SHAPES {
        for n_tokens in contexts {
            ...
            // Q8 quant noise on K and V → same 5e-2 bar as the single-pass Q8 cert.
            let tol = 5e-2;
            ...
            tracing::info!(
                target: "flambeau_bench::sweep_attention_q8_kv_splitk",
                head_dim, n_heads_q, n_heads_kv,
                n_tokens, max_rel, tol,
                "q8 kv decode split-K shape"
            );
        }
    }
    ...
    let cert = Cert {
        ...
        impl_id: "attention_decode_q8_kv_splitk_gfx906".to_string(),
...
fn run_shape(
    ...
    // Quantise K/V to Q8_0 per row (head_dim elements → head_dim/32 blocks),
    // matching the KvCache<Q8Contig> layout [n_tokens, n_heads_kv, head_dim/32].
    ...
    for row in 0..n_rows {
        k_blocks.extend(quantize_row_q8_0(&k_f32[row * head_dim..(row + 1) * head_dim]));
        v_blocks.extend(quantize_row_q8_0(&v_f32[row * head_dim..(row + 1) * head_dim]));
    }
    ...
    let scale = 1.0 / (head_dim as f32).sqrt();
    {
        ...
        let scale_f = scale;
        let window_i: i32 = 0;
        let ring_depth_i: i32 = 0;
        let chunk_base_i: i32 = 0;
        ...
        a1.push(&scale_f);
        a1.push(&window_i);
        a1.push(&ring_depth_i);
        a1.push(&chunk_base_i);
        // Chunk pass: block = head_dim/4 (one thread per int32-packed quad).
        let cfg1 = LaunchCfg {
        ...
        a2.push(&n_chunks_i);
        a2.push(&head_dim_i);
        // Combine pass: block = head_dim (one thread per output element).
        let cfg2 = LaunchCfg {
    ...
    // Reference: dequantise K/V back to F32 (exactly what the kernel reads) and
    // run the F32 attention math — any delta is kernel arithmetic, not quant noise.
    let mut k_rt = vec![0.0f32; kv_len];
    let mut v_rt = vec![0.0f32; kv_len];
    for row in 0..n_rows {
        let bk = &k_blocks[row * nb_per_row..(row + 1) * nb_per_row];
        let bv = &v_blocks[row * nb_per_row..(row + 1) * nb_per_row];
        k_rt[row * head_dim..(row + 1) * head_dim].copy_from_slice(&dequantize_row_q8_0(bk));
        v_rt[row * head_dim..(row + 1) * head_dim].copy_from_slice(&dequantize_row_q8_0(bv));
    }

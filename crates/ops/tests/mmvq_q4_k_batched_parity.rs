//! Parity test — `mmvq_q4_k_batched` vs looping single-row Q4_K MMVQ.
//! Per-block decode is on-the-fly per element (no DP4A), so the two
//! paths differ only in FMA ordering across the slot loop.
...
use flambeau_quant::{BlockQ4K, BlockQ8_1, QK_K};
use half::f16;

fn dev_or_skip() -> Option<HipDevice> {
    if device_count().ok()? < 1 {
        eprintln!("no HIP devices — skipping mmvq_q4_k_batched_parity");
        return None;
    }
...
fn tame_q4_k_scales(bytes: &mut [u8]) {
    let bs = std::mem::size_of::<BlockQ4K>();
    for chunk in bytes.chunks_exact_mut(bs) {
        let d = f16::from_f32((chunk[0] as f32 / 255.0) * 0.05 + 0.005);
        let dmin = f16::from_f32((chunk[1] as f32 / 255.0) * 0.02);
...
fn run_parity(label: &str, shape: Shape, n_slots: usize, seed: u64) -> Result<bool> {
    ...
    let mut w_q4_k_bytes = seeded_u8(
        seed.wrapping_add(1),
        n_superblocks * std::mem::size_of::<BlockQ4K>(),
    );
    tame_q4_k_scales(&mut w_q4_k_bytes);

    let act_f32 = seeded_f32(seed.wrapping_add(2), n_slots * k, 1.0);
    let d_act_f32 = upload(&dev, &act_f32);
    let act_q8_1_bytes = n_slots * (k / 32) * std::mem::size_of::<BlockQ8_1>();
    let d_act_q8_1 = alloc_zeroed(&dev, act_q8_1_bytes);
    quantize_q8_1(&reg, stream, d_act_f32, d_act_q8_1, n_slots * k)?;
    stream.synchronize()?;

    let d_w = upload_bytes(&dev, &w_q4_k_bytes);
    ...
    eprintln!(
        "[mmvq_q4_k_batched_parity] {label} N={n_slots} n_rows={n_rows} k={k} \
    ...
    unsafe {
        ...
        dev.dealloc(d_w, w_q4_k_bytes.len())?;
...
#[test]
fn mmvq_q4_k_batched_parity_sweep() -> Result<()> {
    ...
    for (label, shape, slots) in cases {
        for &n_slots in *slots {
            let pass = run_parity(label, *shape, n_slots, 0x4BEEFCAFE_u64)?;
            if !pass {
                all_pass = false;
            }
        }
    ...
    assert!(
        all_pass,
        "one or more parity cases exceeded the 5e-4 abs-err tolerance"
    );
    Ok(())
}

...
void flambeau_indexed_moe_mmq_q5_0_down_tile8_dp4a_q8_1(
    const flambeau_block_q5_0* __restrict__ w,
    ...
    for (int ib = 0; ib < n_blocks_per_row; ++ib) {
        ...
        if (row_ok) {
            ...
            const flambeau_block_q5_0* wbx = &w[w_row_off];
            w_d = (float) wbx->d;
        ...
        for (int c = 0; c < TILE_N; ++c) {
            ...
            sums[c] += w_d * (d8 * (float) (sumi_nib + 16 * sumi_bit) - 16.0f * s8);
        }
    }

...
/// Upload one MTP tensor to `device`. Norms (F32 1D) go up as-is;
/// linears can be Q8_0 or F32 depending on the converter's choice
/// (current converter writes Q8_0 for linears, F32 for norms).
fn upload_mtp_tensor(
    ...
    // Reuse the project's standard upload path. We can't import
    // `sharded::upload_one` directly (private), so inline the simple
    // case — MTP linears are Q8_0 / F32, no BF16 / MXFP4 / quirky-Q4_1
    // paths to handle.

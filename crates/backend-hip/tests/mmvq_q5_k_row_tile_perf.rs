//! Microbench: Q5_K row-tile vs r2 batched on decode-class shapes.
//! 200 warmup + 1000 measured launches, HIP-event timed.
...
fn bench_one(dev: &HipDevice, n_rows: usize, k: usize, n_slots: usize) -> (f32, f32) {
    assert_eq!(k % QK_K, 0);
    ...
    for i in 0..(n_rows * n_sb_per_row) {
        x_blocks.push(random_q5k_block(0xDEAD_BEEF, i));
    }
    ...
    let d_y_q8_1 = dev
        ...
        .unwrap();
    let d_out = dev.alloc(n_slots * n_rows * 4).unwrap();
    ...
    unsafe {
        ...
        dev.dealloc(d_out, n_slots * n_rows * 4).unwrap();
    }
...
fn perf_q5k_row_tile_vs_r2() {
    ...
    // (n_rows, k, n_slots) — Q5_K decode-class shapes. QKV proj on
    // Qwen3.5-27B-Q5_K: n_rows = 5120 (hidden), k = 5120 (hidden), N decode slots.
    // ffn_gate/up dense: n_rows ~ 13824/TP2 = 6912, k = 5120.
    ...
    eprintln!("\n{:>10} | {:>6} | {:>4} | {:>9} | {:>9} | {:>6}", "n_rows", "k", "N", "K3(µs)", "RT(µs)", "K3/RT");
    eprintln!("{}", "-".repeat(60));
    for (n_rows, k, n) in shapes {
        let (k3_us, rt_us) = bench_one(&dev, n_rows, k, n);
        eprintln!(
            "{:>10} | {:>6} | {:>4} | {:>9.2} | {:>9.2} | {:>5.2}×",
            n_rows, k, n, k3_us, rt_us, k3_us / rt_us
        );
    }
}

//! End-to-end correctness for `flambeau_mmvq_iq3_xxs_q8_1` on MI50 vs CPU
//! dequant + f32 matmul (Q8_1 activation round-trip isolates kernel
//! arithmetic from quant-representation noise).
//!
//! Builds random IQ3_XXS blocks directly (`d`, `qs`, `scs`) — all byte
//! patterns are valid: the codebook + sign-LUT indices are bounded by the
//! 8-bit field widths, and the 4-bit per-ib32 scale lives at the top of
//! `aux32` (random bytes give a uniform scale distribution).
...
use flambeau_quant::{BlockIq3Xxs, BlockQ8_1, QK8_0, QK_K};
use half::f16;
...
fn random_iq3_xxs_block(seed: u64, idx: usize) -> BlockIq3Xxs {
    let bytes = seeded_bytes(seed ^ ((idx as u64).wrapping_mul(0x9E3779B97F4A7C15)), 98);
    // Keep d small. Grid magnitudes reach ~0x3e (62) and IQ3_XXS scales the
    // accumulator by `(0.5 + scale_4b) * 0.5 * d` (max ~7.75 * d). At k=15360
    // a too-large d overflows F32 relative precision.
    let d_scalar = (bytes[0] as f32 / 255.0) * 0.005 + 0.0005;
    let d = f16::from_f32(d_scalar);
    let mut qs = [0u8; QK_K / 4 + QK_K / 8];
    qs.copy_from_slice(&bytes[2..2 + QK_K / 4 + QK_K / 8]);
    BlockIq3Xxs { d, qs }
}
...
fn run_mmvq_iq3_xxs(n_rows: usize, k: usize, seed: u64, kernel_stem: &'static str, entry: &'static str, rows_per_block: usize) -> (Vec<f32>, Vec<f32>) {
    assert_eq!(k % QK_K, 0);
    ...
    let mut x_blocks: Vec<BlockIq3Xxs> = Vec::with_capacity(n_rows * superblocks_per_row);
    for i in 0..(n_rows * superblocks_per_row) {
        x_blocks.push(random_iq3_xxs_block(seed, i));
    }
    let total_elems = n_rows * k;
    let mut x_dequant = vec![0.0f32; total_elems];
    {
        let raw: &[u8] = bytemuck::cast_slice(&x_blocks);
        flambeau_quant::dequantize_into(flambeau_quant::GgmlDType::Iq3Xxs, raw, &mut x_dequant).unwrap();
    }
    ...
    unsafe {
        dev.dealloc(d_x, x_blocks.len() * std::mem::size_of::<BlockIq3Xxs>()).unwrap();
...
#[test]
fn mmvq_iq3_xxs_small() {
    if !maybe_skip() { return; }
    let (got, reference) = run_mmvq_iq3_xxs(4, QK_K, 0xC0FFEE, "mmvq_iq3_xxs", "flambeau_mmvq_iq3_xxs_q8_1", 1);
    let err = max_rel_err(&got, &reference);
    eprintln!("[mmvq_iq3_xxs 4×{QK_K}] err={err:.3e}, tol={:.3e}", tol_for(QK_K));
    assert!(err <= tol_for(QK_K), "err {err:.3e}");
}

#[test]
fn mmvq_iq3_xxs_2048() {
    if !maybe_skip() { return; }
    let k = 2048;
    let (got, reference) = run_mmvq_iq3_xxs(8, k, 0xFEEDFACE, "mmvq_iq3_xxs", "flambeau_mmvq_iq3_xxs_q8_1", 1);
    let err = max_rel_err(&got, &reference);
    eprintln!("[mmvq_iq3_xxs 8×{k}] err={err:.3e}, tol={:.3e}", tol_for(k));
    assert!(err <= tol_for(k), "err {err:.3e}");
}

#[test]
fn mmvq_iq3_xxs_5120() {
    if !maybe_skip() { return; }
    let k = 5120;
    let (got, reference) = run_mmvq_iq3_xxs(16, k, 0x12345678, "mmvq_iq3_xxs", "flambeau_mmvq_iq3_xxs_q8_1", 1);
    let err = max_rel_err(&got, &reference);
    eprintln!("[mmvq_iq3_xxs 16×{k}] err={err:.3e}, tol={:.3e}", tol_for(k));
    assert!(err <= tol_for(k), "err {err:.3e}");
}

#[test]
fn mmvq_iq3_xxs_r2_small() {
    if !maybe_skip() { return; }
    let (got, reference) = run_mmvq_iq3_xxs(4, QK_K, 0xC0FFEE, "mmvq_iq3_xxs_r2", "flambeau_mmvq_iq3_xxs_r2_q8_1", 2);
    let err = max_rel_err(&got, &reference);
    eprintln!("[mmvq_iq3_xxs_r2 4×{QK_K}] err={err:.3e}, tol={:.3e}", tol_for(QK_K));
    assert!(err <= tol_for(QK_K), "err {err:.3e}");
}

#[test]
fn mmvq_iq3_xxs_r2_2048() {
    if !maybe_skip() { return; }
    let k = 2048;
    let (got, reference) = run_mmvq_iq3_xxs(8, k, 0xFEEDFACE, "mmvq_iq3_xxs_r2", "flambeau_mmvq_iq3_xxs_r2_q8_1", 2);
    let err = max_rel_err(&got, &reference);
    eprintln!("[mmvq_iq3_xxs_r2 8×{k}] err={err:.3e}, tol={:.3e}", tol_for(k));
    assert!(err <= tol_for(k), "err {err:.3e}");
}

#[test]
fn mmvq_iq3_xxs_r2_odd_rows() {
    if !maybe_skip() { return; }
    let k = 1024;
    let (got, reference) = run_mmvq_iq3_xxs(7, k, 0xDEADBEEF, "mmvq_iq3_xxs_r2", "flambeau_mmvq_iq3_xxs_r2_q8_1", 2);
    let err = max_rel_err(&got, &reference);
    eprintln!("[mmvq_iq3_xxs_r2 7×{k}] err={err:.3e}, tol={:.3e}", tol_for(k));
    assert!(err <= tol_for(k), "err {err:.3e}");
}

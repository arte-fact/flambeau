...
fn random_block(seed: u64, idx: usize) -> BlockIq2S {
    ...
    scales.copy_from_slice(&bytes[2 + QK_K / 4 + QK_K / 32..]);
...
fn run(
    ...
    dev.default_stream().synchronize().unwrap();
    unsafe {
        dev.dealloc(d_x, blocks.len() * std::mem::size_of::<BlockIq2S>())
            .unwrap();
...
fn iq2_s_small() {
    ...
    let (g, r) = run(
        ...
        0xC0FFEE,
        "mmvq_iq2_s",
        "flambeau_mmvq_iq2_s_q8_1",
        1,
    );
    let e = err(&g, &r);
    eprintln!("[iq2_s 4x{QK_K}] err={e:.3e}");
    assert!(e <= tol(QK_K));
}
#[test]
fn iq2_s_2048() {
    ...
    let e = err(&g, &r);
    eprintln!("[iq2_s 8x2048] err={e:.3e}");
    assert!(e <= tol(2048));
}
#[test]
fn iq2_s_r2_small() {
    ...
    let (g, r) = run(
        ...
        0xC0FFEE,
        "mmvq_iq2_s_r2",
        "flambeau_mmvq_iq2_s_r2_q8_1",
        2,
    );
    let e = err(&g, &r);
    eprintln!("[iq2_s_r2 4x{QK_K}] err={e:.3e}");
    assert!(e <= tol(QK_K));
}
#[test]
fn iq2_s_r2_2048() {
    ...
    let e = err(&g, &r);
    eprintln!("[iq2_s_r2 8x2048] err={e:.3e}");
    assert!(e <= tol(2048));
}
#[test]
fn iq2_s_r2_odd() {
    ...
    let e = err(&g, &r);
    eprintln!("[iq2_s_r2 7x1024] err={e:.3e}");
    assert!(e <= tol(1024));
}

...
const REFERENCE_LOGITS_HEAD16: [f32; 16] = [
    -0.09114712, 0.13210723, -0.35692906, 0.27114975,
    -0.47151846, 0.32448775, -0.38120058, 0.26533583,
    -0.14050573, 0.10671204, 0.14263627, -0.10818739,
    0.35757548, -0.2975785, 0.41441876, -0.3999702,
];
...
fn parity_snapshot_attn_gated_single_token() {
    ...
    let attn = AttnWeights {
        ...
        attn_v: Some(allocs.upload_q8_0(
            &det_signal(kv_width * HIDDEN, 303),
            kv_width,
            HIDDEN,
        )),
        attn_output: allocs.upload_q8_0(
            &det_signal(HIDDEN * q_width, 304),
            HIDDEN,
            q_width,
        ),
    ...
    let logits: Vec<f32>;
    {
        ...
        let delta = ctx.standard_attn(&resid_in, &attn, 0, 0).expect("standard_attn");

//! End-to-end correctness for `flambeau_mmvq_iq2_xs_q8_1` on MI50.
...
use flambeau_quant::{BlockIq2Xs, BlockQ8_1, QK8_0, QK_K};
use half::f16;
...
fn random_block(seed: u64, idx: usize) -> BlockIq2Xs {
    ...
    let mut scales = [0u8; QK_K / 32];
    scales.copy_from_slice(&bytes[2 + 2 * QK_K / 8..2 + 2 * QK_K / 8 + QK_K / 32]);
    BlockIq2Xs { d, qs, scales }
}
...
fn run(rows: usize, k: usize, seed: u64, stem: &'static str, entry: &'static str, rpb: usize) -> (Vec<f32>, Vec<f32>) {
    ...
    let blocks: Vec<BlockIq2Xs> = (0..rows*nsb).map(|i| random_block(seed, i)).collect();
    let mut deq = vec![0.0f32; rows*k];
    flambeau_quant::dequantize_into(flambeau_quant::GgmlDType::Iq2Xs, bytemuck::cast_slice(&blocks), &mut deq).unwrap();
    ...
    dev.default_stream().synchronize().unwrap();
    unsafe { dev.dealloc(d_x, blocks.len() * std::mem::size_of::<BlockIq2Xs>()).unwrap(); dev.dealloc(d_y, y.len() * 4).unwrap();
...
#[test] fn iq2_xs_small() { if !maybe_skip() { return; } let (g,r) = run(4, QK_K, 0xC0FFEE, "mmvq_iq2_xs", "flambeau_mmvq_iq2_xs_q8_1", 1); let e = err(&g,&r); eprintln!("[iq2_xs 4x{QK_K}] err={e:.3e}"); assert!(e <= tol(QK_K)); }
#[test] fn iq2_xs_2048() { if !maybe_skip() { return; } let (g,r) = run(8, 2048, 0xFEEDFACE, "mmvq_iq2_xs", "flambeau_mmvq_iq2_xs_q8_1", 1); let e = err(&g,&r); eprintln!("[iq2_xs 8x2048] err={e:.3e}"); assert!(e <= tol(2048)); }
#[test] fn iq2_xs_r2_small() { if !maybe_skip() { return; } let (g,r) = run(4, QK_K, 0xC0FFEE, "mmvq_iq2_xs_r2", "flambeau_mmvq_iq2_xs_r2_q8_1", 2); let e = err(&g,&r); eprintln!("[iq2_xs_r2 4x{QK_K}] err={e:.3e}"); assert!(e <= tol(QK_K)); }
#[test] fn iq2_xs_r2_2048() { if !maybe_skip() { return; } let (g,r) = run(8, 2048, 0xFEEDFACE, "mmvq_iq2_xs_r2", "flambeau_mmvq_iq2_xs_r2_q8_1", 2); let e = err(&g,&r); eprintln!("[iq2_xs_r2 8x2048] err={e:.3e}"); assert!(e <= tol(2048)); }
#[test] fn iq2_xs_r2_odd() { if !maybe_skip() { return; } let (g,r) = run(7, 1024, 0xDEADBEEF, "mmvq_iq2_xs_r2", "flambeau_mmvq_iq2_xs_r2_q8_1", 2); let e = err(&g,&r); eprintln!("[iq2_xs_r2 7x1024] err={e:.3e}"); assert!(e <= tol(1024)); }

...
void flambeau_indexed_moe_mmq_q5_0_gate_up_tile8_dp4a_q8_1(
    const flambeau_block_q5_0* __restrict__ gate_w,
    const flambeau_block_q5_0* __restrict__ up_w,
    ...
    for (int ib = 0; ib < blocks_per_row_x; ++ib) {
        ...
        if (row_ok) {
            ...
            const flambeau_block_q5_0* gbx = &gate_w[w_row_off];
            const flambeau_block_q5_0* ubx = &up_w[w_row_off];
            g_d = (float) gbx->d;
            u_d = (float) ubx->d;
        ...
        for (int c = 0; c < TILE_N; ++c) {
            ...
            sums_gate[c] += g_d * (d8 * (float) (sumi_g_nib + 16 * sumi_g_bit) - 16.0f * s8);
            sums_up[c]   += u_d * (d8 * (float) (sumi_u_nib + 16 * sumi_u_bit) - 16.0f * s8);
        }
    }

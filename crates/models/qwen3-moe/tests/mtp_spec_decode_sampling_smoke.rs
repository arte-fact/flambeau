//! MTP-5g smoke — drive `forward_speculative_pp_step_sampling` for N
//! macro steps with non-greedy sampling (temp + top-p). Verifies the
//! rejection-sampling path runs end-to-end and produces coherent
//! output. Skipped without the GGUF + 4 HIP devices.
...
use flambeau_qwen3_moe::forward::{
    forward_one_token_pp_logits, forward_speculative_pp_step_sampling,
...
use flambeau_runtime::sampling::{Rng, Sampling};
...
#[test]
fn mtp_spec_decode_sampling_smoke() -> Result<()> {
    ...
    eprintln!("=== MTP-5g spec-decode sampling smoke ===");
    ...
    let sampling = Sampling::top_p(0.8, 0.9);
    ...
    for step in 0..n_macro {
        let (s, h_next) = forward_speculative_pp_step_sampling(
            &model, &mut session, &cluster, &mut decode_scratch, &mut prefill_scratch,
            &mtp, &mtp_scratch,
            output_norm, lm_head,
            last_token, h_for_mtp, position,
            &sampling, &mut rng, &all_tokens,
        )?;
        eprintln!(
            "  macro {step}: pos {position}→{} draft={} {} committed={:?}",
            s.new_position, s.draft,
            if s.accepted { "✓" } else { "✗" },
            s.committed,
        );
    ...
    eprintln!(
        "  wall: {wall_ms:.0} ms, {:.1} ms/token",
        wall_ms / total_tokens.max(1) as f64
    );

...
#[test]
fn forward_one_token_pp_synthetic_2rank() -> Result<()> {
    let n = device_count().unwrap_or(0);
    if n < 2 {
        eprintln!("need ≥ 2 HIP devices for PP smoke — got {n}, skipping");
        return Ok(());
    }
    ...
    // sanity: layer 0 → rank 0 (GDN), layer 1 → rank 1 (full-attn)
    assert_eq!(assignment.rank_for(0), RankId(0));
    assert_eq!(assignment.rank_for(1), RankId(1));
    assert!(cfg.is_recurrent(0));
    assert!(!cfg.is_recurrent(1));

    let layout = {
        // We don't have a GGUF here — synthesize a layout stub (the layout
        // is carried through only for `total_bytes()` diagnostics; no
        // forward op reads it).
        ModelLayout {
    ...
    // Zero Q4_K / Q8_0 matmul weights → every delta zero → last rank's LM
    // head matmul is zero × zero → argmax lane 0.

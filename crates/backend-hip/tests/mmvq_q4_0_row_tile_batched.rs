//! Parity for `mmvq_q4_0_row_tile_batched` vs `mmvq_q4_0_batched`.
//! Both kernels consume identical Q4_0 + Q8_1 inputs and must produce
...
fn run_both(n_rows: usize, k: usize, n_slots: usize, seed: u64) -> Outs {
    ...
    let base_bytes = kernels::hsaco("mmvq_q4_0_batched").unwrap();
    let rt_bytes = kernels::hsaco("mmvq_q4_0_row_tile_batched").unwrap();
    ...
    let mut w_blocks: Vec<BlockQ4_0> = Vec::with_capacity(n_rows * n_blocks_per_row);
    for i in 0..(n_rows * n_blocks_per_row) {
        w_blocks.push(random_q4_0_block(seed, i));
    }
    ...
    unsafe {
        dev.dealloc(d_w, w_blocks.len() * std::mem::size_of::<BlockQ4_0>())
            .unwrap();
...
fn assert_close(label: &str, base: &[f32], rt: &[f32], k: usize) {
    ...
    eprintln!(
        "[{label}] max_rel_err={worst:.3e} at idx={worst_idx} (base={}, rt={}), tol={tol:.3e}",
...
fn parity_n3_k4096_asym_rows() {
    ...
    // Tail row count: n_rows=37 → grid=10 blocks, last block has only 1 active row.
    let outs = run_both(37, 4096, 3, 0xFEED_FACE);
    assert_close("n3 k4096 n_rows=37", &outs.baseline, &outs.row_tile, 4096);
}

#[test]
fn parity_n4_k800_tail_partial() {
    if !maybe_skip() {
        return;
    }
    // K=800 → n_blocks_per_row=25 = 1 full outer iter + tail of 9 blocks.
    let outs = run_both(16, 800, 4, 0xFADE_BEEF);
    assert_close("n4 k800 n_rows=16", &outs.baseline, &outs.row_tile, 800);
}

#[test]
fn parity_n4_k2048_gdn_ssm_out_shape() {
    if !maybe_skip() {
        return;
    }
    // Qwen3.6-27B-Q4_0 / pp2tp2 GDN ssm_out shape (per-rank slice):
    // n_rows=2048 (hidden), k=4096 (local_d_inner). This is the actual
    // shape the row-tile kernel replaces in the dispatch path.
    let outs = run_both(2048, 4096, 4, 0x27B_BEEF);
    assert_close(
        "n4 ssm_out_2048x4096",
        &outs.baseline,
        &outs.row_tile,
        4096,
    );
